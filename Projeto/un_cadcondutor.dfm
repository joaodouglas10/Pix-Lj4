object frm_cadcondutor: Tfrm_cadcondutor
  Left = 419
  Top = 157
  Caption = 'Cadastro de Condutor - Casa F'#225'tima - vers'#227'o: 1.0'
  ClientHeight = 360
  ClientWidth = 644
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesigned
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 9
    Top = 51
    Width = 44
    Height = 13
    Caption = 'C'#211'DIGO'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 149
    Top = 51
    Width = 61
    Height = 13
    Caption = 'CONDUTOR'
    FocusControl = nome
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 419
    Top = 51
    Width = 47
    Height = 13
    Caption = 'VEICULO'
    FocusControl = veiculo
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 9
    Top = 97
    Width = 36
    Height = 13
    Caption = 'PLACA'
    FocusControl = placa
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label7: TLabel
    Left = 149
    Top = 118
    Width = 109
    Height = 13
    Caption = 'Pesquisa por Motorista'
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 644
    Height = 37
    Align = alTop
    BevelInner = bvLowered
    Color = clTeal
    ParentBackground = False
    TabOrder = 4
    ExplicitLeft = 5
    ExplicitTop = 2
    ExplicitWidth = 628
    object novo: TSpeedButton
      Left = 8
      Top = 5
      Width = 80
      Height = 29
      Caption = 'Novo'
      Glyph.Data = {
        
        9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = novoClick
    end
    object editar: TSpeedButton
      Left = 88
      Top = 5
      Width = 80
      Height = 29
      Caption = 'Alterar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = editarClick
    end
    object excluir: TSpeedButton
      Left = 168
      Top = 5
      Width = 80
      Height = 29
      Caption = 'Excluir'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = excluirClick
    end
    object gravar: TSpeedButton
      Left = 319
      Top = 5
      Width = 80
      Height = 29
      Caption = 'OK'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = gravarClick
    end
    object cancelar: TSpeedButton
      Left = 400
      Top = 5
      Width = 80
      Height = 29
      Caption = 'Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = cancelarClick
    end
    object sair: TSpeedButton
      Left = 540
      Top = 5
      Width = 80
      Height = 29
      Caption = 'Sair'
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = sairClick
    end
  end
  object nome: TDBEdit
    Left = 149
    Top = 67
    Width = 264
    Height = 21
    CharCase = ecUpperCase
    DataField = 'CONDUTOR'
    DataSource = Dtm_Entrega.Ds_Condutor
    ReadOnly = True
    TabOrder = 1
  end
  object veiculo: TDBEdit
    Left = 419
    Top = 67
    Width = 151
    Height = 21
    CharCase = ecUpperCase
    DataField = 'VEICULO'
    DataSource = Dtm_Entrega.Ds_Condutor
    ReadOnly = True
    TabOrder = 2
  end
  object placa: TDBEdit
    Left = 9
    Top = 113
    Width = 134
    Height = 21
    CharCase = ecUpperCase
    DataField = 'PLACA'
    DataSource = Dtm_Entrega.Ds_Condutor
    ReadOnly = True
    TabOrder = 3
  end
  object DBGrid1: TDBGrid
    Left = 9
    Top = 144
    Width = 624
    Height = 208
    DataSource = Dtm_Entrega.Ds_Condutor
    DrawingStyle = gdsClassic
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'ID_CONDUTOR'
        Title.Caption = 'C'#211'DIGO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CONDUTOR'
        Width = 160
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VEICULO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PLACA'
        Width = 98
        Visible = True
      end>
  end
  object DBNavigator1: TDBNavigator
    Left = 537
    Top = 113
    Width = 96
    Height = 25
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    TabOrder = 6
  end
  object reg: TDBEdit
    Left = 9
    Top = 67
    Width = 134
    Height = 21
    DataField = 'ID_CONDUTOR'
    DataSource = Dtm_Entrega.Ds_Condutor
    ReadOnly = True
    TabOrder = 0
  end
  object Edit1: TEdit
    Left = 264
    Top = 115
    Width = 268
    Height = 21
    Hint = 'Click aqui para iniciar a pesquisa'
    CharCase = ecUpperCase
    ParentShowHint = False
    ShowHint = True
    TabOrder = 7
    OnChange = Edit1Change
  end
end
